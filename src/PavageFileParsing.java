...
public class PavageFileParsing {
	...
	//	try{
			 //Création du lecteur de fichier
	          ...
	          //Génération du tableau selon toutes les positions possibles de chaque piece
	...
	public static int[][] generateLinesFromPieces(Piece[] pieces, Grille grid) {
		...
		int[][] tab=new int[pieces.length*8*nbColTypePavage][nbColTypePavage+pieces.length]; //Représente notre future matrice
		//toutes les lignes ne seront pas utilisées il faudra recopier à la fin
		//Pire des cas : chaque piece se retourne 8 fois, et on peut la mettre partout (pas très réaliste mais bon...)
				...
				for(int i=0;i<grid.nbLigne;i++){
					for(int j=0;j<grid.nbCol;j++){
						...
						if(grid.admissiblePositionForPiece(p, i, j)){
							//Délicat : il faut obtenir les cases couvertes, jusque là rien de bien méchant
							//MAIS il faut aussi transformer ces coordonnées x,y en une seule coord sur la ligne, entre 0 et numberOfValidCases !
